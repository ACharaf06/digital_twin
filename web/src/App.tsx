...
import type { StageAction } from './components/MascotStage'

const MascotStage = lazy(() => import('./components/MascotStage'))
...
export default function App() {
  ...
  useEffect(()=>{
    document.querySelectorAll<HTMLElement>('.studio-header,.conversation-space,.mascot-tools,.studio-footer').forEach(element=>{element.inert=!entered})
  },[entered])
  useEffect(
    () => () => {
      window.clearTimeout(bubbleTimer.current)
  ...
  return (
    <main className={`twin-studio ${entered ? 'has-entered' : 'is-booting'}`}>
      <a className="skip-link" href="#twin-input" onClick={()=>{setEntered(true);setView('chat')}}>
      ...
      <header className="studio-header">
        ...
        >
          ...
          <span>
            charaf<span className="brand-period">.</span>
          </span>
        ...
        <div className="studio-location">
          <span className="live-dot" /> FROM FRANCE, WITH CURIOSITY
        </div>
        <button
      ...
      <div className="scene-intro">
        <span className="micro-label">AI ENGINEER / DIGITAL ALTER EGO</span>
        <h1>
      ...
      <div className="mascot-caption">
        ...
        <span>
          CHARAF ACHIR<span>{moodLabels[mood]}</span>
        </span>
      </div>
      ...
      <footer className="studio-footer">
        ...
        <a href="mailto:charaf.achir6@gmail.com">
