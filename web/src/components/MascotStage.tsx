...
import generatedMascotSource from '../../../assets/3d/charaf-studio.glb?url'
import handSource from '../../../assets/3d/studio-hand.glb?url'
import portraitSource from '../../../assets/video/centre.png'
...
export default function MascotStage({
  ...
  const state = useRef({ entered, mood, action, wireframe, onInteraction })
  ...
  useEffect(() => {
    ...
    const mountRig = (nextRig: MascotRig, source: 'generated' | 'procedural') => {
      if (disposed) {
        nextRig.dispose()
        return
      }
    ...
    const loadProceduralFallback = () => {
      new THREE.TextureLoader().load(
        ...
        (texture) => {
          ...
          mountRig(createMascot(texture), 'procedural')
        },
        undefined,
        () => {
          if (!disposed) {
    ...
    Promise.all([loader.loadAsync(generatedMascotSource), loader.loadAsync(handSource)])
      .then(([gltf, hands]) =>
        mountRig(createGeneratedMascot(gltf.scene, hands.scene), 'generated'),
      )
    ...
    let hoverAt = 0
    const neutralGaze = new THREE.Vector2()
    let width = 1,
      height = 1
    let framing = 0
    ...
    const onPointerUp = (event: PointerEvent) => {
      if (Math.hypot(event.clientX - pointerDown.x, event.clientY - pointerDown.y) > 7 || !rig)
        return
      onPointerMove(event)
      scene.updateMatrixWorld(true)
      raycaster.setFromCamera(pointer, camera)
      const hit = raycaster.intersectObjects(
        [...(rig.hitTargets ?? [rig.root]), laptop, toy],
        true,
      )[0]
      if (!hit) return
    ...
    const animate = (now: number) => {
      ...
      if (state.current.action.id !== previousAction) {
        ...
        actionAt = elapsed
        spinStart = rig?.root.rotation.y ?? 0
        ...
        if (currentAction === 'reset') {
          ...
          resetAt = elapsed
          if (rig && !rig.motion) rig.root.rotation.y = 0
        }
      }
      if (resetAt >= 0) {
      ...
      const envelope = actionActive
        ? Math.min(1, actionTime * 4) *
          Math.min(
            1,
            ((currentAction === 'dance' ? 5 : currentAction === 'wave' ? 3.2 : 2) - actionTime) * 3,
          )
        : 0
      if (rig) {
        ...
        const thinking = state.current.mood === 'thinking'
        const speaking = state.current.mood === 'speaking'
        const listening = state.current.mood === 'listening'
        const dancing = currentAction === 'dance' ? envelope : 0
        const waving = currentAction === 'wave' ? envelope : 0
        const presenting = currentAction === 'present' ? envelope : 0
        ...
        if (elapsed > blinkingAt && !reduced) {
          blinkStart = elapsed
          blinkingAt = elapsed + 3.5 + Math.sin(elapsed) * 1.1
        }
        const blinkTime = elapsed - blinkStart
        const blink = blinkTime > 0 && blinkTime < 0.19 ? Math.sin((blinkTime / 0.19) * Math.PI) : 0
        const speech =
          speaking && !reduced
            ? 0.25 + Math.abs(Math.sin(time * 12) * Math.sin(time * 7)) * 0.75
            : 0
        rig.updateFace(blink, speech, interacting ? neutralGaze : gaze)
        if (!rig.motion) {
          rig.head.rotation.y = THREE.MathUtils.damp(
            rig.head.rotation.y,
            interacting ? 0 : gaze.x * 0.22,
            4,
            dt,
          )
          rig.head.rotation.x = THREE.MathUtils.damp(
            rig.head.rotation.x,
            -gaze.y * 0.15 + (thinking ? -0.1 : 0) + (speaking ? Math.sin(time * 3) * 0.035 : 0),
            4,
            dt,
          )
          rig.head.rotation.z = THREE.MathUtils.damp(
            rig.head.rotation.z,
            thinking ? 0.12 : Math.sin(time * 0.65) * 0.017 + Math.sin(time * 4) * dancing * 0.09,
            4,
            dt,
          )
          rig.body.rotation.z = Math.sin(time * 0.7) * 0.012 + Math.sin(time * 5) * dancing * 0.12
          rig.body.rotation.x = listening ? 0.025 : Math.sin(time * 1.3) * 0.007
          rig.body.scale.y = 1 + Math.sin(time * 1.6) * 0.004
          rig.root.position.y = Math.abs(Math.sin(time * 5)) * dancing * 0.08
          rig.root.scale.set(1, 1, 1)
          if (currentAction === 'jump' && actionActive) {
            const jump = Math.min(1, actionTime / 1.1)
            rig.root.position.y += Math.sin(jump * Math.PI) * 0.75
            const squash = Math.sin(Math.min(1, actionTime / 1.45) * Math.PI * 2) * 0.08
            rig.root.scale.set(1 - squash * 0.3, 1 + squash, 1 - squash * 0.3)
          }
          if (currentAction === 'spin' && actionActive)
            rig.root.rotation.y = spinStart + ease(actionTime / 1.7) * Math.PI * 2
          rig.arms[0].shoulder.rotation.z =
            -0.1 - waving * 2.1 - dancing * (0.7 + Math.sin(time * 5) * 0.6) - presenting * 0.7
          rig.arms[0].shoulder.rotation.x = -0.02 - waving * 0.12
          rig.arms[0].elbow.rotation.z =
            waving * (-0.3 + Math.sin(time * 10) * 0.23) - presenting * 0.45
          rig.arms[0].elbow.rotation.x = -0.07 - presenting * 0.65
          rig.arms[0].hand.rotation.z = waving * Math.sin(time * 10) * 0.25
          rig.arms[1].shoulder.rotation.z =
            0.11 +
            dancing * (0.65 - Math.sin(time * 5) * 0.5) +
            (thinking ? 0.42 : speaking ? 0.18 : 0)
          rig.arms[1].shoulder.rotation.x = thinking ? -0.5 : speaking ? -0.18 : 0
          rig.arms[1].elbow.rotation.x = THREE.MathUtils.damp(
            rig.arms[1].elbow.rotation.x,
            thinking ? -2 : speaking ? -0.65 + Math.sin(time * 3) * 0.13 : -0.16,
            5,
            dt,
          )
          rig.legs[0].hip.rotation.x = Math.sin(time * 5) * dancing * 0.2
          rig.legs[1].hip.rotation.x = -Math.sin(time * 5) * dancing * 0.2
          rig.legs[0].hip.rotation.z = -0.025 - dancing * 0.09
          rig.legs[1].hip.rotation.z = 0.025 + dancing * 0.09
        }

        host.dataset.mood = state.current.mood
        if (!rig.motion) host.dataset.action = actionActive ? currentAction : 'idle'
  ...
  return (
    <div className="mascot-stage" ref={hostRef} data-fallback={failed || undefined}>
      {failed && (
        <div className="mascot-fallback">
          <img src={portraitSource} alt="Charaf's original cartoon mascot" />
          <p>3D is unavailable in this browser. I'm still here to chat.</p>
        </div>
      )}
    </div>
  )
